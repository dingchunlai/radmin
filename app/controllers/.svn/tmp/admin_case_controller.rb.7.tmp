  ...
  def articles_to_cases_index
    ...
    if current_staff.member_of?("兼职文章编辑")
      ...
      @pages, @cases = paginate :hejia_case, :per_page => @per_page,
        :conditions => con, :order => "ID desc"
    else
      @pages, @cases = paginate :hejia_case, :per_page => @per_page,
        :conditions => ["(TYPE_ID != ? OR TYPE_ID IS NULL) and CREATEDATE>'2011-01-01 00:00:01'", -5], :order => "ID desc"
  ...
  def search_article_cases
    ...
    if @editor_id.blank? && @state.blank?
      ...
      sql = "SELECT h.ID as ID, h.NAME as NAME, h.CREATEDATE as CREATEDATE, h.INTRODUCE as INTRODUCE, h.MAINPHOTO as MAINPHOTO," +
        " h.STATE as STATE FROM HEJIA_CASE h where #{use_time} ((TYPE_ID != -5 OR TYPE_ID IS NULL) and CREATEDATE>'2011-01-01 00:00:01') and h.NAME like '%#{@case_name}%';"
    else
      ...
      unless @state.blank?
        case @state
        when "0"
          sql = "SELECT ID, NAME, CREATEDATE, INTRODUCE, MAINPHOTO, STATE FROM HEJIA_CASE where ((TYPE_ID != ? OR TYPE_ID IS NULL) and CREATEDATE>'2011-01-01 00:00:01') and STATE = 1 and NAME like '%#{@case_name}%';"
        else
