...
function page_index_list(recordcount, pagesize, curpage, curlist, listsize){
    ...
    if (curpage==1)
      ...
      str += "<a href='javascript:gotopage(" + (curpage-1) + ");' title='跳转至前一页'>前一页</a>　";

	if (curlist==1)
	str += "<< ";
	else
	str += "<a href='javascript:page_index_list(recordcount, pagesize, curpage, cur_list-1, listsize);' title='显示前一批页码'><<</a> ";
	...
	for (var i = startpage; i <= endpage; i++)
	{
		...
		if (i == curpage)
			...
			str += ("<a href='javascript:gotopage(" + i + ");' title='跳转至第" + i + "页'>" + i + "</a> ");
	}

	if (curlist==Math.ceil(pagecount / listsize))
	str += ">>　";
	else
	str += "<a href='javascript:page_index_list(recordcount, pagesize, curpage, cur_list+1, listsize);' title='显示后一批页码'>>></a>　";

    if (curpage==pagecount)
      str += "后一页 "
    else
      str += "<a href='javascript:gotopage(" + (curpage+1) + ");' title='跳转至后一页'>后一页</a> ";
